...
import 'package:mitopup/presentation/screens/auth/login.dart';

import '../../../config/config.dart';

// Todo: Aqui tenia el conuntry
...
class LoginPage extends StatefulWidget {
  ...
  _LoginPageState createState() => _LoginPageState(userId: userId, name: name);
}
...
class _LoginPageState extends State<LoginPage> {
  ...
  _LoginPageState({required this.userId, required this.name});
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isNotEmpty) {
          ...
          minDigits = selectedCountry!.digits;
          maxDigits = selectedCountry!.digits;
  ...
  void _openCountrySelectionPage() async {
    ...
    if (selected != null) {
      setState(() {
        ...
        minDigits = selected.digits;
        maxDigits = selected.digits;
      });
      ...
      if (phoneNumber.length > selected.digits) {
        phoneNumberController.text = phoneNumber.substring(0, selected.digits);
      }
      _checkPhoneNumberField();
    }
  }
  ...
  void _navigateToLogin() async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse(
          'http://5.78.79.129:8080/app.getUserName?idPais=$idPais&phoneNumber=$phoneNumber',
        ),
      );
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => LoginScreen(
                idPais: idPais,
                // Pasar el idPais a LoginScreen
                phoneNumber: phoneNumber,
                // Pasar el phoneNumber a LoginScreen
                userId: userId,
